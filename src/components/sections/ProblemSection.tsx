...
const Stat = ({ value, label, color, delay }: { value: React.ReactNode; label: string; color: string; delay: number }) => (
  ...
  >
    <span style={{ fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: 'clamp(40px, 5vw, 64px)', color, lineHeight: 1 }}>
      {value}
    </span>
    <span style={{ fontSize: 10, letterSpacing: '0.15em', color: '#555', fontWeight: 700, textTransform: 'uppercase', marginTop: 8 }}>{label}</span>
  </motion.div>
);
        ...
        {isActive && (
          <motion.div key="p" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <motion.h2 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}
              style={{ fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: 'clamp(28px, 3.5vw, 52px)', color: '#f5f0e8', margin: 0, lineHeight: 1.15 }}>
            ...
            <motion.blockquote initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}
              ...
              <p style={{ fontFamily: "'Playfair Display', serif", fontStyle: 'italic', color: 'rgba(245,240,232,0.85)', fontSize: 'clamp(14px, 1.4vw, 19px)', margin: 0, lineHeight: 1.6 }}>
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}
              style={{ fontFamily: "'Playfair Display', serif", fontStyle: 'italic', color: '#00a862', fontWeight: 700, fontSize: 'clamp(14px, 1.4vw, 18px)', margin: 0 }}>
