...
const TimeBar = ({ label, pct, color, delay }: { label: string; pct: number; color: string; delay: number }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
    <span style={{ fontSize: 13, fontWeight: 700, color: '#666', textTransform: 'uppercase', letterSpacing: '0.1em', width: 80, flexShrink: 0 }}>{label}</span>
    ...
    <span style={{ fontSize: 13, fontWeight: 900, color: '#f5f0e8', width: 40, textAlign: 'right' }}>
...
const DrinkPill = ({ label, pct, color, delay }: { label: string; pct: number; color: string; delay: number }) => (
  <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay }}
    ...
    <span style={{ fontSize: 14, color: '#f5f0e8', fontWeight: 700 }}>{label}</span>
        ...
        {isActive && (
          <motion.div key="o" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
              ...
              <p style={{ fontSize: 13, letterSpacing: '0.3em', color: '#444', fontWeight: 900, textTransform: 'uppercase', margin: '0 0 4px' }}>Time of Day</p>
              ...
              <p style={{ fontSize: 14, color: '#444', fontStyle: 'italic', margin: '4px 0 0' }}>45% of orders happen before noon.</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}>
              <p style={{ fontSize: 13, letterSpacing: '0.3em', color: '#444', fontWeight: 900, textTransform: 'uppercase', margin: '0 0 10px' }}>Drink Type Split</p>
